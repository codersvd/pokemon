import App from '../../app';
import {POKEMON_LIST_ALL, POKEMON_SEARCH} from './actions';
...
export default [
  ...
  /**
   * @description middleware for action POKEMON_SEARCH
   * get data from API and subscribe store
   */
  store => next => action => {
    next(action);
    if (action.type === POKEMON_SEARCH) {
      let linkApi = "http://pokeapi.co/api/v2";
      console.log(action);
      if (!sessionStorage.getItem("pokemon" + action.name)) {
        fetch(linkApi + "/pokemon/" + action.name).then(res => {
          if (res.status >= 400){
            action.pokemon = {count: 1, results: []};
            next(action);
            throw new Error("Server error");
          }
          return res.json();
          ...
          .then(res => {
            let poke = new Pokemon(res);
            console.log(poke);
            action.pokemon = {count: 1, results: [poke]};
            sessionStorage.setItem("pokemons" + action.page, JSON.stringify(res));
            next(action);
          }).catch(error => {
            console.log(error);
        ...
        action.pokemon = JSON.parse(sessionStorage.getItem("pokemon" + action.name));
        next(action);
      }
    }
  }
